...
2. **演算処理**: `x.sum(axis = self.axis, keepdims = self.keepdims)`の結果を`y`に格納
...
| 変数名 | 説明 | 使用箇所 | 補足 |
| :--- | :--- | :--- | :--- |
| `gy` | `broadcast_to`を用いて、勾配の形状を入力時の形状にリシェイプした値を格納 | リシェイプ処理後 |  | 

1. **リシェイプ処理**: `util.reshape_sum_backward(gy, axis = self.axis, keepdims = self.keepdims)`で`gy`の値をリシェイプし`gy`に格納する
